...
import { assertEventDocument, createRuntime, hydrateEvent, type Runtime } from "@hooksmith/runtime";
...
import { type CliReport, createReport, type EventExecutionReport, type EventInput, formatReport, inferRoutingOutcome, toEventReport } from "./report.ts";
...
const runCommand = new Command()
  ...
  .option("-c, --config <path:string>", "Config file.", { default: "hooksmith.config.ts" })
  .option("--format <format:report-format>", "Report format.", { default: "table" })
  ...
  .action(async (options, eventFile, ...eventFiles) => {
    ...
    if (inputs.filter((path) => path === "-").length > 1) throw new Error("run accepts stdin at most once.");
    const configFile = resolve(options.config);
    const config = await loadConfig(configFile);
    const runtime = createRuntime(config, { log: stderrLogger });
    const report = await processBounded(runtime, { eventFiles: inputs, configFile, format: options.format, plan: options.plan ?? false, allowEmpty: options.allowEmpty ?? false });
...
const helpCommand = new Command()
  ...
  .helpOption(false)
  .noGlobals()
  .option("-h, --help", "", { hidden: true, action: () => { throw new Error("Unknown option: --help."); } })
...
export function usage(): string { return cli.getHelp(); }
...
async function processBounded(runtime: Runtime, options: RunCliOptions): Promise<CliReport> {
  ...
  for (const path of paths) {
    ...
    try { documents = await loadEventDocuments(path); } catch (error) {
      eventIndex++;
      events.push(inputFailure({ source, index: eventIndex, sourceIndex: 1 }, error));
      continue;
    }
    for (let sourceIndex = 0; sourceIndex < documents.length; sourceIndex++) {
      eventIndex++;
      events.push(await processDocument(runtime, documents[sourceIndex], { source, index: eventIndex, sourceIndex: sourceIndex + 1 }, options.plan));
    }
  }
  if (events.length === 0 && !options.allowEmpty) {
    events.push(inputFailure({ source: "run", index: 1, sourceIndex: 0 }, new Error("No events were resolved from the supplied inputs.")));
  }
...
async function processStream(runtime: Runtime): Promise<number> {
  ...
  for await (const line of readLines(Deno.stdin.readable)) {
    ...
    eventIndex++;
    const input: EventInput = { source: "stdin", index: eventIndex, sourceIndex: lineNumber };
    let eventReport: EventExecutionReport;
    try { eventReport = await processDocument(runtime, JSON.parse(line), input, false); } catch (error) { eventReport = inputFailure(input, error); }
...
async function processDocument(runtime: Runtime, document: unknown, input: EventInput, plan: boolean): Promise<EventExecutionReport> {
  let event: Event;
  try { assertEventDocument(document); event = hydrateEvent(document); } catch (error) { return inputFailure(input, error); }
  try {
    const report = plan ? await runtime.plan(event) : await runtime.process(event);
    return { input, event: report.event, outcome: report.outcome ?? inferRoutingOutcome(report), results: report.results, success: report.success };
  } catch (error) {
    return { input, event: toEventReport(event), outcome: "failed", results: [], success: false, error: { stage: "runtime", message: errorMessage(error) } };
  }
}

function inputFailure(input: EventInput, error: unknown): EventExecutionReport {
  return { input, outcome: "rejected", results: [], success: false, error: { stage: "input", message: errorMessage(error) } };
}

export async function loadConfig(path: string): Promise<Config> {
  const module = await import(toFileUrl(path).href);
  if (!("default" in module)) throw new Error("Config module must have a default export.");
  return module.default as Config;
}

function inputSource(path: string): string { return path === "-" ? "stdin" : path; }
...
async function writeStdout(value: string): Promise<void> { await Deno.stdout.write(new TextEncoder().encode(value)); }
